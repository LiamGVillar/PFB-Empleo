...
def informacion_pbi():
    ...
    def transformar_datos(df, encoder):
    # Aplica las transformaciones definidas en el encoder manualmente
        for columna, mapeo in encoder.items():
            if mapeo is not None:
                if callable(mapeo):  # Si es una función (como la de mapear experiencia)
                    df[columna] = df[columna].apply(mapeo)
                elif isinstance(mapeo, dict):  # Si es un diccionario de mapeo
                    df[columna] = df[columna].map(mapeo)
                else:
                    df[columna] = mapeo  # Si es un valor fijo como el de 'vacaciones'
        return df


    ## Cargar modelo y transformadores
    model = joblib.load('/home/bosser/Escritorio/proyectofinal/predicciones/modelo_empleo.pkl')
    encoder = joblib.load('/home/bosser/Escritorio/proyectofinal/clustering/encoder.pkl')  # Cargar el encoder
    scaler = joblib.load('/home/bosser/Escritorio/proyectofinal/predicciones/minmax_scaler.pkl')  # Cargar el scaler
    imputer = joblib.load('/home/bosser/Escritorio/proyectofinal/predicciones/knn_imputer.pkl')  # Cargar el imputador

    # Cargar df para asignar valores al user_input
    df_imp = pd.read_csv("/home/bosser/Escritorio/proyectofinal/predicciones/df_imputado.csv")

    # Interfaz de usuario en Streamlit
    st.title('Predicción de Salario de Ofertas de Empleo')

    # Inputs para que el usuario ingrese las características de la oferta
    ubicacion = st.selectbox('Ubicación:', ['Madrid', 'Barcelona', 'Sevilla', 'Valencia'])  # ejemplo
    experiencia = st.slider('Experiencia requerida (años):', 0, 30, 5)
    categoria = st.selectbox('Categoría del trabajo:', ['TI', 'Marketing', 'Ventas', 'Consultoría'])  # ejemplo
    nivel = st.selectbox('Nivel de trabajo:', ['Junior', 'Semi-Senior', 'Senior'])

    # Crear el dataframe del input del usuario
    ...
    # Imputar valores faltantes (usando valores aleatorios del dataframe df_imp)
    for col in df_imp.columns:
        if col not in user_input.columns:
            user_input[col] = np.random.choice(df_imp[col].dropna().values)

    # Transformar las variables categóricas usando el encoder manual
    user_input_transformed = transformar_datos(user_input, encoder)

    # Asegúrate de que las columnas en user_input_transformed estén en el mismo orden que el scaler espera
    columnas_esperadas = scaler.feature_names_in_  # Obtén las columnas del scaler (orden esperado)
    
    # Reorganizar el DataFrame user_input_transformed para que las columnas estén en el orden correcto
    user_input_transformed = user_input_transformed[columnas_esperadas]
    # Aplicar el scaler (para las variables numéricas)
    ...
    user_input_imputed = np.delete(user_input_imputed, 0, axis=1)
    ...
    if st.button('Predecir Salario'):
        ...
        # Desescalar solo el salario predicho
        ...
        st.write(f'El salario estimado es: {salario_predicho_desescalado[0]:,.2f}k €')

...
def informacion_pbi():
    st.title("Calculadora de Cluster para Creación de Ofertas de Empleo")
    ...
    max_salario = df_general['salario_media'].max().astype(int)
    max_salario
    ...
    st.subheader('Introduce los datos de la oferta que deseas publicar para predecir el cluster.')
...
def clustering_clasificacion():
    ...
    # Mostrar el DataFrame ordenado y el conteo de clusters si show_original es False
    if not st.session_state.show_original:
        st.write("DataFrame ordenado por Silhouette Score:")
        st.dataframe(st.session_state.df_to_show)

        st.write("Conteo de clusters:")
    ...
    if st.button("Ordenar por Silhouette Score"):
        # Ordenar el DataFrame
        df_ordenado = df_cluster.sort_values(by="silhouette_score", ascending=False)
        
        # Actualizar el estado de sesión con el DataFrame ordenado
        st.session_state.df_to_show = df_ordenado
        
        # Ocultar el DataFrame original y mostrar el ordenado y el conteo de clusters

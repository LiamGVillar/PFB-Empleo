...
def informacion_pbi():
    # Cargar el dataframe y los objetos guardados
    df_general = pd.read_csv("/home/bosser/Escritorio/proyectofinal/clustering/df_imputado.csv")
    print(df_general.columns)
    objects_loaded = joblib.load('/home/bosser/Escritorio/proyectofinal/predicciones/transformers_and_model.pkl')
    ...
    # Selección de tecnologías y habilidades
    ...
    # Asignar la moda de 'vacaciones' (columna numérica)
    vacaciones_moda = df_general['vacaciones'].mode()[0]

    # Crear el dataframe de entrada del usuario
    user_input = pd.DataFrame({
        'variable': [0],  # Asegúrate de que el nombre sea correcto
        ...
        'personas_a_cargo': [0],  # Llenado aleatorio de personas a cargo
        'experiencia': [experiencia],  # Usar la experiencia proporcionada por el usuario
        'idioma': [idioma],
        'ciudad': [ciudad]  # Usar la ciudad proporcionada por el usuario
    }, index=[0])

    # Añadir las tecnologías y habilidades seleccionadas
    ...
    # Aplicar el encoder manualmente sobre las columnas correspondientes
    ...
    # Reorganizar el DataFrame para que las columnas estén en el orden correcto

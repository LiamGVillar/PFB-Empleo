...
################################################### Tercera Grafica
    ...
    SELECT 
        ciudad,
        (salario_desde + salario_hasta) / 2 AS salario_promedio
    FROM ofertas
    WHERE salario_desde IS NOT NULL 
    AND salario_hasta IS NOT NULL;
    """

    df = pd.read_sql(consulta_sql, db)

    # Obtenemos la lista única de ciudades
    ciudades = df['ciudad'].unique().tolist()

    # Crear figura base
    fig = go.Figure()

    # Añadir traza inicial (todas las ciudades)
    fig.add_trace(
        go.Box(
            x=df['ciudad'],
    ...
    # Crear trazas para cada ciudad (invisibles inicialmente)
    ...
    for i, ciudad in enumerate(ciudades):
        ...
        visibilidad[i+1] = True  # +1 por la traza inicial
    ...
    st.write("La visualización interactiva permite examinar los rangos retributivos mediante un desplegable de localizaciones. Madrid y Barcelona muestran valores atípicos superiores, aunque sus distribuciones centrales se alinean con el resto de regiones.")
...
################################################# Cuarta grafica
    ...
    st.write("""El diagrama de caja muestra la relación entre tecnologías clave (Azure, Python, SQL, SAP EWM, Peoplesoft), niveles de experiencia (eje x) y distribución salarial (eje y). Peoplesoft, software de gestión empresarial y recursos humanos, presenta los salarios más altos en roles senior (+5 años), mientras que Azure destaca en posiciones junior (<2 años). Los rangos salariales son homogéneos en experiencia media,
              evidenciando una correlación directa entre años de experiencia exigidos y remuneración ofertada.
""")
...
pages = {
    ...
    "About us" : about_us,
    "Calculadora de salario 💎" : predictor  # Asegúrate de agregar la página de la calculadora aquí
}

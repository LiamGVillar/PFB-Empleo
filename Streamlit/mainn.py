...
def predictor ():
    ...
    df_general = pd.read_csv("Calculadora/df_imputado.csv")
    print(df_general.columns)
    objects_loaded = joblib.load('Calculadora/transformers_and_model.pkl')
    ...
    if st.button('Predecir Salario'):
        ...
        for i in range(5):  # Repite 4 veces la animación (puedes ajustar)
            puntos = "." * i
            mensaje_placeholder.write(f":reloj_de_arena: Calculando el salario adecuado según los parámetros{puntos}")
            time.sleep(1)  # Pequeña pausa para el efecto
        # Mostrar el resultado final
        mensaje_placeholder.write(f":marca_de_verificación_blanca: La estimación del salario para una oferta con las características introducidas es: {salario_predicho_desescalado[0] * 1000:,.0f} €")
...
# Condicional para mostrar la función "predictor"
if st.session_state.page == "Calculadora de salario 💎":
    predictor()  # Aquí se ejecuta la función que contiene la calculadora

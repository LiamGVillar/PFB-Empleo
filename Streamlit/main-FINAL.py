...
def predictor ():
    ...
    objects_loaded = joblib.load('/home/bosser/Documentos/PFB-Empleo/Calculadora/transformers_and_model.pkl')
...
def predecir_cluster():
    # Título de la app
    st.title("Predicción de Cluster para Oferta de Empleo")
    ...
    # Subtítulo de la sección
    st.subheader('Introduce los datos de la oferta para predecir a qué cluster pertenece.')
    ...
    # Añadir tecnología y habilidades
    tecnologias = st.multiselect(
        'Tecnologías requeridas:',
        ['sap ewm', 'python', 'sql', 'AWS', 'JavaScript', 'Python', 'peoplesoft', 'Docker', 'aws', 'React'],
        key="tecnologias_multiselect"
    ...
    # Crear el diccionario con las características de la nueva oferta de empleo
    nueva_oferta = {
        'vacaciones': 15,  # Ejemplo: 15 días de vacaciones
        'teletrabajo': 1,  # Ejemplo: 1 para teletrabajo disponible
        'jornada_tipo': 1,  # Ejemplo: jornada completa
        'funciones': 10,  # Ejemplo: 10 funciones en el trabajo
        'contrato_tipo': 2,  # Ejemplo: contrato indefinido
        'nivel_profesional': 3,  # Ejemplo: nivel profesional 3
        'formacion_minima': 2,  # Ejemplo: formación mínima nivel 2
        'personas_a_cargo': 0,  # Ejemplo: sin personas a cargo
        'experiencia': 5,  # Ejemplo: 5 años de experiencia
        'num_tecnologias': 3,  # Ejemplo: 3 tecnologías usadas
        'num_habilidades': 8  # Ejemplo: 8 habilidades
    }

    # Convertir el diccionario a un DataFrame
    nueva_oferta_df = pd.DataFrame([nueva_oferta])

    # Asegurarse de que todas las columnas esperadas estén presentes
    for col in expected_columns:
        if col not in nueva_oferta_df.columns:
            # Si falta una columna, se agrega con valor 0 (o valor por defecto)
            nueva_oferta_df[col] = 0

    # Asegurarse de que las columnas estén en el mismo orden que el modelo espera
    nueva_oferta_df = nueva_oferta_df[expected_columns]

    # Realizar la predicción con el modelo de clasificación
    cluster_pred = clf.predict(nueva_oferta_df)

    # Predicción con el modelo DBSCAN (si lo necesitas)
    dbscan_pred = dbscan.predict(nueva_oferta_df)

    # Mostrar el resultado
    mensaje_placeholder = st.empty()  # Espacio reservado para el mensaje
    for i in range(5):  # Repite 4 veces la animación (puedes ajustar)
        puntos = "." * i
        mensaje_placeholder.write(f"⌛: Calculando el cluster adecuado según los parámetros{puntos}")
        time.sleep(1)  # Pequeña pausa para el efecto

    # Mostrar los resultados finales
    mensaje_placeholder.write(f"✅: La nueva oferta de empleo ha sido clasificada en el cluster por el clasificador: {cluster_pred[0]}")
    mensaje_placeholder.write(f"📊: Predicción del modelo DBSCAN: {dbscan_pred[0]}")
